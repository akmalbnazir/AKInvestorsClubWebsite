...
export default function MorphDock({ session }: { session: Sess }) {
  ...
  if (session) {
    ...
    if (ADMIN_ROLES.includes(session.role)) items.push({ label: "Admin", href: "/admin" });
  ...
  const applyDockSpace = useCallback(() => {
    ...
    root.style.setProperty("--dock-space", `${el.getBoundingClientRect().height + 16}px`);
  }, [open]);
  ...
  return (
    <>
      ...
      {/* ALWAYS-VISIBLE MENU PILL when dock is CLOSED — centered via full-width wrapper */}
      <AnimatePresence>
        {!open && (
          <div className="fixed bottom-1.5 inset-x-0 z-[41] flex justify-center pointer-events-none">
            <motion.button
              ...
              onClick={() => { setOpen(true); cancelHide(); onAnyInteraction(); }}
              initial={{ y: 16, opacity: 0, filter: "blur(4px)" }}
              animate={{ y: 0, opacity: 1, filter: "blur(0px)" }}
              exit={{ y: 16, opacity: 0, filter: "blur(4px)" }}
              transition={{ type: "spring", stiffness: 300, damping: 26 }}
              className="
                pointer-events-auto rounded-full px-4 py-1 text-xs
                bg-black/60 border border-emerald-400/30 shadow-neon text-ak-text/90
              "
            >
              ▲ Menu
            </motion.button>
        ...
        {open && (
          ...
          >
            <motion.div
              layout
              className="
                mx-auto w-[min(100vw-16px,1200px)]
                backdrop-blur bg-black/60 border border-emerald-400/30 rounded-2xl
                px-2 py-2 shadow-neon
            ...
            >
              {items.map((it, i) => (
                <motion.button
                  ...
                  animate={pulse === i ? { boxShadow: "0 0 30px rgba(0,255,136,0.6)" } : {}}
